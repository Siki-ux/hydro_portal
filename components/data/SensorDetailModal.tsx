...
async function getDataPoints(sensorUuid: string, token: string, datastream: string) {
    ...
    const url = `${apiUrl}/things/${sensorUuid}/datastream/${datastream}/observations?limit=100`;
...
export default function SensorDetailModal({
    ...
    const confirmDelete = () => { onDelete(String(sensor.uuid || sensor.id)); onClose(); };
                    ...
                    <div className="flex gap-2">
                        {isDeleting ? (
                            <>
                                ...
                                <button onClick={confirmDelete} className="px-3 py-1 bg-red-500 hover:bg-red-600 text-white rounded text-sm transition-colors">Yes</button>
                                <button onClick={cancelDelete} className="px-3 py-1 bg-white/10 hover:bg-white/20 text-white rounded text-sm transition-colors">No</button>
                            </>
                        ) : (
                            <>
                                <Link
                                ...
                                <button onClick={handleDelete} className="flex items-center gap-1 px-3 py-1 bg-red-500/10 hover:bg-red-500/20 text-red-400 rounded text-sm transition-colors border border-red-500/20">
                        ...
                        <div className="grid grid-cols-1 gap-4 text-sm">
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div className="bg-white/5 p-3 rounded-lg border border-white/5">
                                    ...
                                    <div className="text-white">{displaySensor.device_type || "Generic"}</div>
                                </div>
                            </div>
